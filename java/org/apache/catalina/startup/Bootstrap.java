...
public final class Bootstrap {
    ...
    // /common：类库可以被tomcat和所有webapps下的应用程序使用
    ClassLoader commonLoader = null;

    // /server:类库可以被tomcat使用
    ClassLoader catalinaLoader = null;

    // /shared：类库可以被webapps下的所有应用使用，但是对tomcat不可见
    ClassLoader sharedLoader = null;

    // 注：放在webapps/{app-name}/WEB-INFO下的类库只可以被该应用使用，其它web应用和tomcat无法使用
    ...
    /**
     * 初始化上面说的几个类加载器
     * 在catalina.properties文件默认是没有配置server.loader和shared.loader的，所以它们都是共享
     * 的common的类加载器，及所有的类库都放到了lib目录中
     */
    ...
    public void init() throws Exception {
        ...
        // 这个可以关注一下，双亲委派机制的双向实现（子加载器委托父加载器，反过来父加载器如何去委托子加载器去加载类呢？）
